...
function organizzeItems() {
  ...
  [...items].each((item) => {
    ...
    result.add({ date: date, description: description, value: value });
  })
  return result
}
...
function nubankItems() {
  let items = new Set();
  items.add({ date: "13/03/19", description: "Kanui Com 2/4", value: -41.95 });
  items.add({ date: "31/03/19", description: "teste", value: -12345.67 });
  return items
}
